...
AbstractClass._forDB = function(data) {
    var res = {}, NoSQL = ['mongodb', 'mongoose'];
    Object.keys(data).forEach(function(propName) {
        if (this.whatTypeName(propName) === 'JSON' || data[propName] instanceof Array) {
            if (NoSQL.indexOf(this.schema.adapter.name) === -1) {
...
/**
 * Update records
 * @param {Object} params - update conditions
 * @param {Object} data - data to update
 * @param {Object} options - data to update
 * @param {Function} callback - callback called with (err)
 */
AbstractClass.update = function update(params, data, options, callback) {
    if (stillConnecting(this.schema, this, arguments))
        return;
